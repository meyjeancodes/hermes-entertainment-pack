...
import PokemonAgentWidget from "@/components/PokemonAgentWidget";
  ...
  return (
    <div className="flex flex-col gap-8 h-full">
      ...
      <div className="flex flex-col items-stretch gap-6">
        <Card className="overflow-hidden border-border bg-background-elevated relative w-full">
          <CardContent className="p-0">
            <div className="relative px-0">
              {/* Wood base — wider, richer gradient */}
              <div className="absolute -bottom-4 left-4 right-4 h-6 bg-gradient-to-b from-amber-950 via-amber-900 to-amber-950 rounded-xl blur-[2px] shadow-lg" />
              <div className="absolute -bottom-2 left-6 right-6 h-1 bg-amber-700/60 rounded-full" />

                  {/* TV housing — beefier bezel with depth layers */}
                <div className="relative bg-gradient-to-b from-slate-800 via-slate-900 to-slate-950 rounded-3xl p-2 md:p-3 shadow-xl border-2 border-slate-600 ring-1 ring-slate-700/50">
                  ...
                  <div className="relative bg-slate-950 rounded-2xl p-2 shadow-[inset_0_4px_16px_rgba(0,0,0,0.7),0_6px_20px_rgba(0,0,0,0.4)] border-2 border-slate-700 ring-inset ring-1 ring-slate-800">
      ...
      <div className="flex flex-col items-center">
        <PokemonAgentWidget selectedGameId={selectedGameId} />
      </div>
...
function YouTubePlayer({ channel, isPlaying, volume, isMuted }: { channel: Channel; isPlaying: boolean; volume: number; isMuted: boolean }) {
  ...
  useEffect(() => {
    ...
    const createPlayer = () => {
      ...
      let videoId = '';
      const isAutoChannel = channel.id === 'ch5' || channel.id === 'ch6' || channel.id === 'ch7' || channel.id === 'ch8';
      let playerVars: Record<string, any> = {
        controls: 0,
        autoplay: isAutoChannel ? 1 : 0,
        mute: 0,
        loop: 1,
        modestbranding: 1,
        rel: 0,
